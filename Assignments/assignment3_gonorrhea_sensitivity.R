...
gonorrhea$gender.numb <- ifelse( test = gonorrhea$Gender == "Female",
                              yes = 0, no = 1)
plot(x = gonorrhea$gender.numb, y = gonorrhea$sensitivity, ylim=c(0.8, 1.0), xlim=c(0, 1),
...
lines(x = gonorrhea$gender.numb[Swab.Symptomatic], y = gonorrhea$Sensitivity[Swab.Symptomatic],
      type = "o", lty = "solid", col = "black")

Swab.Asymptomatic <- gonorrhea$Specimen == "Swab" &
  gonorrhea$Symptoms_Status == "Asymptomatic"
lines(x = gonorrhea$gender.numb[Swab.Asymptomatic], y = gonorrhea$Sensitivity[Swab.Asymptomatic],
      type = "o", lty = "solid", col = "blue")

Urine.Symptomatic <- gonorrhea$Specimen == "Urine" &
  gonorrhea$Symptoms_Status == "Symptomatic"
lines(x = gonorrhea$gender.numb[Urine.Symptomatic], y = gonorrhea$Sensitivity[Urine.Symptomatic],
      type = "o", lty = "solid", col = "green")

Urine.Asymptomatic <- gonorrhea$Specimen == "Urine" &
  gonorrhea$Symptoms_Status == "Asymptomatic"
lines(x = gonorrhea$gender.numb[Urine.Asymptomatic], y = gonorrhea$Sensitivity[Urine.Asymptomatic],
...
mod.fit.inter <- glm(formula=Sensitivity ~ Gender + Specimen + Symptoms_Status + Specimen:Symptoms_Status, 
               family = binomial(link = logit), data = gonorrhea, weights=Sample_size)
